...
if __name__ == "__main__":
    ...
    if question == "3.3" or question == "3.4":
        ...
        quantiles  = np.round( np.quantile(X, [0.05, 0.25, 0.5, 0.75, 0.95]), decimals=6) # Requires numpy version > 1.15
        
        print ("\n-----------------------")
        print ("Question 5.1.2\n")
        print ("Quantiles [0.05 0.25 0.5 0.75 0.95] = " + str(quantiles))
        ...
        min_mean = names[np.argmin(means)]
        max_mean = names[np.argmax(means)]
        min_var  = names[np.argmin(variances)]
        max_var  = names[np.argmax(variances)]

        print ("\n-----------------------")
        print ("Question 5.1.3\n")
        print ("Region with minimum mean     = " + str(min_mean))
        print ("Region with maximum mean     = " + str(max_mean))
        print ("Region with minimum variance = " + str(min_var))
        print ("Region with maximum variance = " + str(max_var))
        ...
        #print (str(model.splitModel.splitVariable) + " > " + str(model.splitModel.splitValue))
    ...
    elif question == "6.5plot":
        # this is to generate the classification boundary plot for question 6.5
