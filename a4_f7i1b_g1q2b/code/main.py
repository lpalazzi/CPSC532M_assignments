...
if __name__ == "__main__":
    ...
    if question == "2":
        ...
        modelL1 = LogisticRegression(penalty="l1", fit_intercept=False).fit(XBin, yBin)
        modelL2 = LogisticRegression(penalty="l2", fit_intercept=False).fit(XBin, yBin)
        print("FOR MODEL L1:")
        print("Training error %.3f" % utils.classification_error(modelL1.predict(XBin),yBin))
        ...
        print("\nFOR MODEL L2:")
        print("Training error %.3f" % utils.classification_error(modelL2.predict(XBin),yBin))
        ...
        # One-vs-all logistic regression (implementation)
        model = linear_model.logLinearClassifier(maxEvals=500, verbose=0)
        model.fit(XMulti, yMulti)
        print("\nOne-vs-all Logistic Regression, assignment implementation")
        print("Training error %.3f" % utils.classification_error(model.predict(XMulti), yMulti))
        print("Validation error %.3f" % utils.classification_error(model.predict(XMultiValid), yMultiValid))

        # One-vs-all logistic regression (scikit-learn)
        model = LogisticRegression(C=99999, fit_intercept=False)
        model.fit(XMulti, yMulti)
        print("\nOne-vs-all Logistic Regression, scikit-learn")
        print("Training error %.3f" % utils.classification_error(model.predict(XMulti), yMulti))
        print("Validation error %.3f" % utils.classification_error(model.predict(XMultiValid), yMultiValid))
        ...
        model = linear_model.softmaxClassifier(maxEvals=500)
        model.fit(XMulti, yMulti)
        print("\nSoftmax Logistic Regression, assignment implementation")
        print("Training error %.3f" % utils.classification_error(model.predict(XMulti), yMulti))
        print("Validation error %.3f" % utils.classification_error(model.predict(XMultiValid), yMultiValid))

        # Softmax logistic regression (scikit-learn)
        model = LogisticRegression(multi_class='multinomial', solver='lbfgs', C=99999, fit_intercept=False)
        model.fit(XMulti, yMulti)
        print("\nSoftmax Logistic Regression, scikit-learn")
        print("Training error %.3f" % utils.classification_error(model.predict(XMulti), yMulti))
        print("Validation error %.3f" % utils.classification_error(model.predict(XMultiValid), yMultiValid))